...
installChaincode () {
        ...
        VERSION=${3:-0.1}
        set -x
        peer chaincode install -n kvote -v ${VERSION} -l ${LANGUAGE} -p ${CC_SRC_PATH} &> log.txt
        res=$?
        set +x
        cat log.txt
...
instantiateChaincode () {
        ...
        if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
                set -x
                peer chaincode instantiate -o orderer0.kvote.com:7050 -C $CHANNEL_NAME -n kvote -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["init",""]}' -P "OR ('Org1MSP.peer')"
                res=$?
                set +x
        else
                set -x
                peer chaincode instantiate -o orderer0.kvote.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n kvote -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["init",""]}' -P "OR ('Org1MSP.peer')" &> log.txt
                res=$?
                set +x
        fi
        verifyResult $res "Chaincode instantiation on peer${PEER}.org${ORG} on channel '$CHANNEL_NAME' failed"
        echo "===================== Chaincode Instantiation on peers on channel '$CHANNEL_NAME' is successful ===================== "
	    sleep $DELAY
        echo
}
